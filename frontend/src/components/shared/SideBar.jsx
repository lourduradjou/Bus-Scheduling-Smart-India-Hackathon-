...
const SideBar = () => {
    ...
    return (
        <div className='min-h-screen'>
            {/* Hamburger Icon */}
            {!isSidebarOpen && (
                <div className='fixed top-5 left-7 z-50 cursor-pointer text-white' onClick={toggleSidebar}>
            ...
            >
                        ...
                        {SIDE_BAR_LINKS.map((link) => (
                            <React.Fragment key={link.key}>
                                <li
                                    className={`rounded-md flex gap-4 ${
                                        ...
                                        activeLink === link.key
                                            ...
                                            : themeClasses[link.theme] || 'text-gray-300 hover:bg-[#6c757d5e] hover:duration-200'
                                    }`}
                                    onClick={() => {
                                        if (link.subDivisons) {
                                ...
                                >
                                    ...
                                    {link.subDivisons && (
                                        <span className='text-lg ml-2'>{openSubmenu === link.key ? '▼' : '▶'}</span>
                                    )}
                                </li>
                                    ...
                                    <ul className='ml-8 flex flex-col'>
                                        {link.subDivisons.map((subLink) => (
                                            <li
                                                ...
                                                onClick={() => handleLinkClick(subLink.key, subLink.path)}
                                            >
                ...
                {/* Logout Button */}
                <div className={isSidebarOpen ? `mb-20 ml-8 w-2/3` : ' w-full flex justify-center mb-20'}>
                    <li
                        className={`rounded-md flex items-center gap-4 p-3 cursor-pointer text-lg ${
                            activeLink === LOGOUT_LINK.key
                                ? 'bg-[#0ea5e9] text-[#fff]'
                                : themeClasses[LOGOUT_LINK.theme] || 'text-gray-300 hover:bg-[#6c757d] hover:duration-200'
                        }`}
                        onClick={() => handleLinkClick(LOGOUT_LINK.key, LOGOUT_LINK.path)}
                    >
                        <div className='text-2xl'>{LOGOUT_LINK.icon}</div>
                        {isSidebarOpen && <span className='font-medium'>{LOGOUT_LINK.label}</span>}
                    </li>
                </div>
            </div>
        </div>
