...
def dynamic_programming_scheduling(crews, buses):
    ...
    for bus in buses:
        for shift in ["morning", "afternoon", "evening", "night"]:
            ...
            if shift_details and shift_details["shift_start"] is not None and shift_details["shift_end"] is not None and not shift_details["crew"]:
                bus_shifts.append({
                    "bus_id": bus["id"],
                    "shift_name": shift,
    ...
    for i, shift in enumerate(bus_shifts):
        ...
        for crew in crew_objects:
            if crew.can_work(shift['shift_start'], shift['shift_end'], shift_duration) and crew.shifts[shift['shift_name']] == 1:
                dp[i]['assigned'] = True
                assignments.append({'bus': shift['bus_id'], 'crew': crew.id, 'shift': shift['shift_name']})
                crew.assign(shift['shift_end'], shift_duration)
                break

        if not dp[i]['assigned']:
            notAssigned += 1
            notAssignedBuses.append(shift['bus_id'] + " (" + shift['shift_name'] + ")")
            #assignments.append({'bus': shift['bus_id'], 'crew': None, 'shift': shift['shift_name']}) #not needed i guess
